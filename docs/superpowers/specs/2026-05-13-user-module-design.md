...
Códigos de recuperação de senha devem ter prazo de expiração configurável. Links de convite também devem respeitar o limite de usos. Após expiração ou esgotamento do limite, devem ser invalidados automaticamente pelo sistema.

### RNF05 — Auditabilidade

Toda ação sensível do módulo de usuários deve gerar um registro de auditoria imutável contendo: identificador do ator, perfil do ator, alvo da ação, tipo da ação e timestamp. As ações auditáveis incluem: cadastro, login, logout, promoção de perfil, concessão e revogação de permissão, suspensão, exclusão e etc.
...
O sistema deve ter loggers que realizam o rastreio da requisição do usuário de forma limpa e concisa

### RNF02 — LoggersDb

O sistema deve registrar loggers especializados no banco de dados. Esses loggers devem registrar fluxos importantes, identificando a requisição e seus dados respeitando a LGPD
